...
我们前文 [我写了首诗，把二分搜索变成了默写题](https://appktavsiei5995.pc.xiaoe-tech.com/detail/i_627cce93e4b0812e17989cfa/1) 详细介绍了二分搜索的细节问题，探讨了「搜索一个元素」，「搜索左侧边界」，「搜索右侧边界」这三个情况，教你如何写出正确无 bug 的二分搜索算法。
...
**我们前文 [我写了首诗，把二分搜索变成了默写题](https://appktavsiei5995.pc.xiaoe-tech.com/detail/i_627cce93e4b0812e17989cfa/1) 详细探讨了上述问题，对这块还不清楚的读者建议复习前文**，已经搞清楚基本二分搜索算法的读者可以继续看下去。
...
![img](images/1-20230421211608968.jpeg)
...
![img](images/2-20230421211608973.jpeg)
...
![img](images/3-20230421211608970.jpeg)
...
![img](images/title1-20230421211608978.png)
...
> info：为什么 `f(x)` 的返回值是 `long` 类型？因为你注意题目给的数据范围和 `f` 函数的逻辑。`piles` 数组中元素的最大值是 10^9，最多有 10^4 个元素；那么当 `x` 取值为 1 时，`hours` 变量就会被加到 10^13 这个数量级，超过了 `int` 类型的最大值（大概 2x10^9 这个量级），所以这里用 `long` 类型避免可能出现的整型溢出。
...
![img](images/4-20230421211608969.jpeg)
...
> tip：我这里采用的是左闭右开的二分搜索写法，关于这个算法中的细节问题，前文 [二分搜索算法详解](https://appktavsiei5995.pc.xiaoe-tech.com/detail/i_627cce93e4b0812e17989cfa/1) 进行了详细分析，这里不展开了。
...
肯定有读者会尝试使用前文 [二分搜索算法详解](https://appktavsiei5995.pc.xiaoe-tech.com/detail/i_627cce93e4b0812e17989cfa/1) 介绍的两端都闭的二分搜索框架来写解法，其实简单，只要修改 `right` 的初始值和 `right` 更新的逻辑即可：
...
![img](images/title2-20230421211608978.png)
...
![img](images/5-20230421211608986.jpeg)
...
![img](images/title-20230421211608988.png)
...
这个题目有点类似前文一道经典动态规划题目 [高楼扔鸡蛋](https://appktavsiei5995.pc.xiaoe-tech.com/detail/i_6298795de4b01a4852072fa7/1)，题目比较绕，又是最大值又是最小值的。
